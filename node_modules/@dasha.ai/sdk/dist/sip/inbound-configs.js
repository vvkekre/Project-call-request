...
async function toInboundConfig(dto, groupApi) {
    var _a;
    return {
        ipAcls: (_a = dto.ipACLs) !== null && _a !== void 0 ? _a : undefined,
        applicationName: dto.applicationName,
        priority: dto.priority,
        groupName: dto.groupId ? (await groupApi.getGroupById(dto.groupId)).name : undefined,
        uri: dto.uri,
    };
}
/**
...
async function deleteConfig(name, options) {
    ...
    await endpointApi.deleteConfig(name, options);
}
...
async function createConfig(name, config, options) {
    ...
    const group = config.groupName ? await groupApi.getGroupByName(config.groupName) : undefined;
    const newConfig = await endpointApi.createConfig({ ...config, name, groupId: group === null || group === void 0 ? void 0 : group.id });
    return await toInboundConfig(newConfig, groupApi);
}
...
async function listConfigs(options) {
    ...
    const endpointApi = new rest.SipEndpointApi(account, log);
    const groupApi = new rest.GroupApi(account, log);
    const configs = await endpointApi.listConfigs();
    const entries = await Promise.all(configs.map(async (config) => [config.name, await toInboundConfig(config, groupApi)]));
 ...
 */
async function updateConfig(name, changes, options) {
    var _a;
    const account = (_a = options === null || options === void 0 ? void 0 : options.account) !== null && _a !== void 0 ? _a : (await account_1._getEffectiveAccount({ _log: log_1.log }));
    const log = log_1.log.child({ account: account_1.anonymizeAccount(account) });
    const endpointApi = new rest.SipEndpointApi(account, log);
    const groupApi = new rest.GroupApi(account, log);
    const group = changes.groupName ? await groupApi.getGroupByName(changes.groupName) : undefined;
    const config = await endpointApi.updateConfig(name, { ...changes, groupId: group === null || group === void 0 ? void 0 : group.id });
    return await toInboundConfig(config, groupApi);
}
