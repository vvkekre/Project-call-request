...
class ProfilerApi extends base.Api {
    ...
    async download(req, cancelToken, onProgress) {
        ...
        const cancelSource = axios_1.default.CancelToken.source();
        cancelToken === null || cancelToken === void 0 ? void 0 : cancelToken._onCancel(this.log, () => cancelSource.cancel());
        const { data, headers } = await this.api.apiV1ProfilerDownloadGet(options.start, options.end, options.applicationId, options.clustersNumber, options.clusterAlgorithm, {
            cancelToken: cancelSource.token,
            responseType: "stream",
        });
    ...
    async isAvailableToDownload(req, cancelToken) {
        ...
        cancelToken === null || cancelToken === void 0 ? void 0 : cancelToken._onCancel(this.log, () => cancelSource.cancel());
        const options = formatRequest(req);
        const { data: status } = await this.api.apiV1ProfilerStatusGet(options.start, options.end, options.applicationId, options.clustersNumber, options.clusterAlgorithm, { cancelToken: cancelSource.token });
