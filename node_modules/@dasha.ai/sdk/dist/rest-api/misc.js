...
class MiscApi extends base.Api {
    ...
    async getAuthServerUrl() {
        const response = await this.api.apiV1MiscAuthServerGet({
    ...
    async startTrial() {
        ...
        const response = await this.axios.get("/api/account/trial", {
    ...
    async getAudioClientAuthToken(request) {
        const { customerId, applicationName, groupId } = request;
        const response = await this.axios.post("/api/v1/audio/auth", { customerId, applicationName, groupId }, {
            responseType: "text",
            validateStatus: (status) => status < 400,
        });
        return response.data;
