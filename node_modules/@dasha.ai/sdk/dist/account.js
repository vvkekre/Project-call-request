...
async function writeAccountConfig(log, config) {
    validateAccountConfig(log, config);
    const configContent = JSON.stringify(config, undefined, 2);
    const fs = await Promise.resolve().then(() => __importStar(require("fs"))).catch(() => undefined);
    const path = await Promise.resolve().then(() => __importStar(require("path"))).catch(() => undefined);
...
async function readAccountConfig(log) {
    const fs = await Promise.resolve().then(() => __importStar(require("fs"))).catch(() => undefined);
    const path = await Promise.resolve().then(() => __importStar(require("path"))).catch(() => undefined);
    if (fs !== undefined && path !== undefined) {
        // we are in node
        ...
        let configContent;
        try {
            configContent = await fs.promises.readFile(configPath, { encoding: "utf-8" });
        }
        catch (error) {
            if (error.code === "ENOENT") {
                // config not found; creating
                await writeAccountConfig(log, defaultAccountConfig);
...
async function listAccounts() {
    ...
    const config = await readAccountConfig(log);
 ...
 */
async function setAccount(name, account, options) {
    const log = log_1.log.child({ label: "accounts" });
    const config = await readAccountConfig(log);
    ...
    await writeAccountConfig(log, config);
}
 ...
 */
async function renameAccount(oldName, newName, options) {
    const log = log_1.log.child({ label: "accounts" });
    const config = await readAccountConfig(log);
 ...
 */
async function getCurrentAccountName() {
    const log = log_1.log.child({ label: "accounts" });
    const config = await readAccountConfig(log);
 ...
 */
async function setCurrentAccountName(name) {
    const log = log_1.log.child({ label: "accounts" });
    const config = await readAccountConfig(log);
...
/** @internal */
async function _getEffectiveAccount(options) {
    var _a, _b;
