...
class ProcessingClient extends base.Client {
    ...
    registerInstanceStream(request) {
        return new base.Stream((stream) => {
            ...
            grpcStream.on("data", (grpcMessage) => {
                const { error, success, progress, jobNotification, jobAdditionalInformation } = grpcMessage.toObject();
                if (error !== undefined) {
                    stream.emit("error", log_1.logAndReturn(this.log, new errors.DeploymentError(error.errorCode, error.errorDetails)));
                    stream.cancel();
                }
                if (success !== undefined) {
                    const inputSchema = this.tryParseJSON(success.inputDataSchema);
                    const outputSchema = this.tryParseJSON(success.outputDataSchema);
                    stream.emit("success", success.appId, success.instanceId, success.applicationName, inputSchema, outputSchema);
                }
                ...
                if (jobAdditionalInformation !== undefined) {
                    if (!jobAdditionalInformation.data)
                        throw new errors.InternalError();
                    const { jobStarted, jobEnded } = jobAdditionalInformation.data;
                    const additionalInformation = {
                        jobStarted: jobStarted ? new Date(jobStarted) : undefined,
                        jobEnded: jobEnded ? new Date(jobEnded) : undefined,
                    };
                    stream.emit("jobAdditionalInformation", jobAdditionalInformation.assignedJobId, jobAdditionalInformation.customerJobId, additionalInformation);
                }
            });
            return grpcStream;
        });
    ...
    /*
      Try to parse JSON.
      Returns undefined on JSON parse errors
    */
    tryParseJSON(text) {
        try {
