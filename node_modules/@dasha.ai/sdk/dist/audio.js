 ...
 */
async function fromFetchResponse(response) {
    // TODO: handle http errors?
    return await fromBlob(await response.blob());
}
 ...
 */
async function fromFile(path, format) {
    const fs = await Promise.resolve().then(() => __importStar(require("fs"))).catch(() => {
        log_1.logAndThrow(log_1.log, new errors.NotImplementedError("only implemented in node"));
    });
    return new Audio(format !== null && format !== void 0 ? format : _guessFormatFromExtension(path), await fs.promises.readFile(path));
}
